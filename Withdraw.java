...
Color c1=new Color(192,192,192);
		...
		l2.setBounds(70,170,250,100);
		add(l2);
		t2=new TextField();
		t2.setBounds(330,70,300,50);
		add(t2);
		t1=new TextField();
		t1.setBounds(330,190,300,50);
		add(t1);
	...
	public void actionPerformed(ActionEvent ae)
	{
		Object ob=ae.getSource();
				if(ob==br1)
			{
				...
				if(Pattern.matches("\\d*",cq) && Pattern.matches("\\d*",j))
				{
					 for(i=0;i<row;i++)
					 { 
						 if(a1[i]==Integer.parseInt(cq))
						 {
							...
							rs5=st.executeQuery("select balance from account_details where account_no="+cq+";");
							 if(rs5.next())
							 {
								...
								bal=bal-Integer.parseInt(j);
							 }
							 st.executeUpdate("update account_details set balance="+bal+" where account_no="+cq+";");
							t1.setText("");
							t2.setText("");
							JOptionPane.showMessageDialog(null,"WITHDRAWN","AXIS",0);
							   break;
						 }
